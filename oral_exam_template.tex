...
\title{CSE Oral Qualifying Exam:\\ Structure-Preserving Numerical Algorithms for Plasma Modeling}
...
\begin{figure}[!htbp]
    \centering
    \begin{tabular}{cccl}
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_pol_A.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_pol_B.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_pol_C.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_pol_colorbar.png}                                                                                                                                                                                                         \\
        \footnotesize $[\mathbf{B}\times\mathbf{J}]_p \in H(\text{div}, \mathcal{T}_{2D})_m$~\eqref{eq:proj_path_A} & \footnotesize $[\mathbf{B}\times\mathbf{J}]_p \in H(\text{curl}, \mathcal{T}_{2D})_m$~\eqref{eq:proj_path_B} & \footnotesize $[\mathbf{B}\times\mathbf{J}]_p \in CG(\mathcal{T}_{2D})_m^2$~\eqref{eq:proj_path_C} & \\
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_tor_A.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_tor_B.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_tor_C.png}                      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_tor_colorbar.png}                                                                                                                                                                                                         \\
        \footnotesize $[\mathbf{B}\times\mathbf{J}]_t \in CG(\mathcal{T}_{2D})_{m}$~\eqref{eq:proj_path_A}          & \footnotesize $[\mathbf{B}\times\mathbf{J}]_t \in DG(\mathcal{T}_{2D})_{m-1}$~\eqref{eq:proj_path_B}         & \footnotesize $[\mathbf{B}\times\mathbf{J}]_t \in CG(\mathcal{T}_{2D})_{m}$~\eqref{eq:proj_path_C} & \\
    \end{tabular}
...
\begin{figure}[!htbp]
    \centering
    \begin{tabular}{cccl}
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/div_B_pol_Hdiv.png}   &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/div_B_pol_Hcurl.png}  &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/div_B_pol_vec_CG.png} &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/div_B_pol_colorbar.png}                                                                                                                         \\
        \footnotesize $D_b \in DG(\mathcal{T}_{2D})_{m-1}$~\eqref{eq:proj_path_A}                     & \footnotesize $D_b \in CG(\mathcal{T}_{2D})_{m}$~\eqref{eq:proj_path_B} & \footnotesize $D_b \in CG(\mathcal{T}_{2D})_{m}$~\eqref{eq:proj_path_C} &
    \end{tabular}
...
\begin{figure}[!htbp]
    ...
    \begin{tabular}{cccl}
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_pol_A.png}        &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_unalign/JxB_pol_A.png}      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align_refine/JxB_pol_A.png} &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_unalign/JxB_pol_colorbar.png}                                   \\
        \footnotesize original                                                                        & \footnotesize w/ perturbation & \footnotesize w/ refinement & \\
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align/JxB_tor_A.png}        &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_unalign/JxB_tor_A.png}      &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align_refine/JxB_tor_A.png} &
        \includegraphics[height=0.25\textheight]{figs/GS_GS_align_refine/JxB_tor_colorbar.png}                              \\
        \footnotesize original                                                                        & \footnotesize w/ perturbation & \footnotesize w/ refinement & \\
    \end{tabular}
    \caption{Comparison of the poloidal and toroidal components of the Lorentz force from different projection paths.}
...
\newpage
\section{Computational Artifact II: Structure-Preserving Finite-Element-Based Particle-In-Cell Method}
\subsection{Description}
This work presents the implementation of a FEM-based particle-in-cell (PIC) method for solving the Vlasov--Poisson equation. The implementation adds a new \textbf{2D2V PIC miniapp} to MFEM that demonstrates explicit particle pushing coupled to a finite-element Poisson solve for self-consistent electric fields on a periodic domain (pull request submitted). The implementation targets plasma physics use cases (e.g., linear Landau damping) and serves as a reference example for coupling MFEM's particle support and FEM-based parallel elliptic solvers. The miniapp features Dirac delta shape functions for particle charge deposition, symplectic structure preservation achieved through the use of compatible finite element spaces following the discrete de-Rham complex and the leap-frog scheme for particle time stepping, parallel particle redistribution across MPI ranks with demonstrated scalability, tested with up to 256 MPI ranks, and optional GLVis visualization for both $\phi$ and $\mathbf{E}$.



\subsection{Governing Equations}
The PIC method solves the Vlasov--Poisson system, 
\begin{equation}
    \frac{\partial f}{\partial t} + \mathbf{v} \cdot \nabla f + \frac{e}{m} \mathbf{E} \cdot \nabla_v f = 0,
\end{equation}
where the distribution function $f$ is approximated by macro-particle representation as 
\begin{equation}
    f \approx \sum_{p=1}^{N_p} \psi_p \delta(\mathbf{x}-\mathbf{x}_p) \delta(\mathbf{p}-\mathbf{p}_p) ,
\end{equation}
where $\psi_p$ is the particle weight, $\mathbf{x}_p$ is the particle position, and $\mathbf{p}_p$ is the particle momentum. Therefore, the charge density with respect to the $\mathbf{x}$ is given by
\begin{equation}
    \rho(\mathbf{x}) = e \int_{\mathbb{R}^2} f(\mathbf{x}, \mathbf{p}) d\mathbf{p} = e \sum_{p=1}^{N_p} \psi_p \delta(\mathbf{x}-\mathbf{x}_p) .
\end{equation}

The electric potential $\phi$ is computed from Poisson's equation,
\begin{equation}
    -\epsilon_0 \Delta\phi(\mathbf{x}) = e \sum_{p=1}^{N_p} \psi_p \delta(\mathbf{x}-\mathbf{x}_p) - e n_0,
\end{equation}
where the vacuum permittivity $\epsilon_0$ is set to 1, $e$ is the particle charge, the particle weight $\psi_p$ is currently set to 1, $\mathbf{x}_p$ is the particle position, and $n_0$ is a constant neutralizing term that enforces global charge neutrality, ensuring that the right-hand side has zero mean and the periodic Poisson problem is well-posed. The electric field is then computed from the potential:
\begin{equation}
    \mathbf{E} = -\nabla \phi.
\end{equation}
...
Here we show our PIC implementation is \emph{symplectic}, i.e., there exists a discrete Hamiltonian associated with our solver, and the spatial and temporal discretizations preseve the symplectic structure by design.

Consider the Poisson equation,
\begin{equation}
    -\epsilon_0 \Delta\phi(\mathbf{x}) = e \sum_{p=1}^{N_p} \psi_p \delta(\mathbf{x}-\mathbf{x}_p) - e n_0.
\end{equation}
We solve the Poisson equation using $H^1$ finite element space, where $\varphi \in H^1(\Omega)$ is a test function,
\begin{equation}
    \epsilon_0 \langle \nabla \varphi, \nabla \phi \rangle = e \sum_{p=1}^{N_p} \psi_p \langle \varphi, \delta(\mathbf{x}-\mathbf{x}_p) \rangle - e n_0 \langle \varphi, 1 \rangle,
\end{equation}
such that we obtain a fully discrete system of equations,
\begin{equation}
    \epsilon_0 \mathbf{K} \hat{\mathbf{\phi}} = \hat{\mathbf{B}}, \label{eq:discrete_poisson_equation}
\end{equation}
where $\mathbf{K}$ is the stiff matrix and $\hat{\mathbf{B}}$ is the right-hand side vector,
\begin{equation}
    \hat{\mathbf{B}}_i = e \sum_{j=1}^{N_p} \psi_j \varphi_i(\mathbf{Q}_j) - e n_0 \int_{\Omega} \varphi_i.
\end{equation}

We want $\mathbf{E} = -\nabla \phi$, by the discrete de-Rham complex,
\begin{displaymath}
    \xymatrix{
        H^1(\Omega) \ar[r]^{\nabla} \ar[d] & H(\text{{\normalfont curl}};\Omega) \ar[d] \\
        CG \ar[r]^{\nabla_h}  & ND,
    }
\end{displaymath}
where $CG$ stands for the continuous Galerkin space and $ND$ standa for the Nedelec space that are used on MFEM.
Therefore, we have the discrete gradient computed as
\begin{equation}
    \langle \mathbf{v}, \hat{\mathbf{E}} + \nabla \phi \rangle = 0, \quad \forall \mathbf{v} \in H(\text{{\normalfont curl}};\Omega). \label{eq:discrete_E_equation}
\end{equation}

Define the discrete Hamiltonian as
\begin{equation}
    H(\{\mathbf{x}_p\},\{\mathbf{p}_p\})
    = \frac{1}{2}\sum_{p=1}^{N_p}\frac{|\mathbf{p}_p|^2}{m_p}
    + \frac{1}{2\epsilon_0}\hat{\mathbf{B}}(\{\mathbf{x}_p\})^T \mathbf{K}^{-1}\hat{\mathbf{B}}(\{\mathbf{x}_p\}),
\end{equation}
where
\begin{equation}
    \hat{\mathbf{B}}_i(\{\mathbf{x}_p\})
    = e\sum_{p=1}^{N_p}\psi_p\,\varphi_i(\mathbf{x}_p) \;-\; e n_0\int_\Omega \varphi_i.
\end{equation}
For the $p$-th particle, $(\mathbf{x}_p, \mathbf{p}_p)$, the canonical dynamics is given by
\begin{align}
    \frac{d\mathbf{x}_p}{dt} & = \frac{\partial H}{\partial \mathbf{p}_p}, \label{eq:canonical_dynamics_x}  \\
    \frac{d\mathbf{p}_p}{dt} & = -\frac{\partial H}{\partial \mathbf{x}_p}. \label{eq:canonical_dynamics_p}
\end{align}
Plug $H$ into \eqref{eq:canonical_dynamics_x},
\begin{equation}
    \frac{d\mathbf{x}_p}{dt} = \frac{\mathbf{p}_p}{m_p}.
\end{equation}

Now, plug $H$ into \eqref{eq:canonical_dynamics_p},
\begin{align}
    \begin{split}
        \frac{d\mathbf{p}_p}{dt} & = -\frac{\partial H}{\partial \mathbf{x}_p}                                                                                                                                                                                                                       \\
                                 & = -\frac{\partial}{\partial \mathbf{x}_p} \left( \frac{1}{2\epsilon_0}\hat{\mathbf{B}}(\{\mathbf{x}_j\})^T \mathbf{K}^{-1}\hat{\mathbf{B}}(\{\mathbf{x}_j\}) \right)                                                                                              \\
                                 & = -\frac{1}{2\epsilon_0}\left(\frac{\partial \hat{\mathbf{B}}}{\partial \mathbf{x}_p}\right)^T \mathbf{K}^{-1}\hat{\mathbf{B}} - \frac{1}{2\epsilon_0}\hat{\mathbf{B}}(\{\mathbf{x}_j\})^T \mathbf{K}^{-1}\frac{\partial \hat{\mathbf{B}}}{\partial \mathbf{x}_p} \\
                                 & = -\frac{1}{\epsilon_0}\left(\frac{\partial \hat{\mathbf{B}}}{\partial \mathbf{x}_p}\right)^T \mathbf{K}^{-1}\hat{\mathbf{B}}
    \end{split}
\end{align}
Here, we use the symmetry of $\mathbf{K}$. Substitute $\hat{\mathbf{B}}$ with $\hat{\mathbf{\phi}}$ from \eqref{eq:discrete_poisson_equation},
\begin{equation}
    \frac{d\mathbf{p}_p}{dt} = -\hat{\mathbf{\phi}}^T \frac{\partial \hat{\mathbf{B}}}{\partial \mathbf{x}_p},
\end{equation}
where
\begin{align}
    \begin{split}
        \frac{\partial \hat{\mathbf{B}}_i}{\partial \mathbf{x}_p} & = \frac{\partial}{\partial \mathbf{x}_p} \left( e\sum_{j=1}^{N_p}\psi_j\,\varphi_i(\mathbf{x}_j) \;-\; e n_0\int_\Omega \varphi_i \right) \\
                                                                & = \frac{\partial}{\partial \mathbf{x}_p} \left( e\sum_{j=1}^{N_p}\psi_j\,\varphi_i(\mathbf{x}_j) \right) \\
                                                                & = e \psi_p \nabla \varphi_i(\mathbf{x}_p),
    \end{split}
\end{align}
so
\begin{align}
    \begin{split}
        \frac{d\mathbf{p}_p}{dt} &= -\hat{\mathbf{\phi}}^T \frac{\partial \hat{\mathbf{B}}}{\partial \mathbf{x}_p}\\
        &= -e \psi_p \hat{\mathbf{\phi}}^T \nabla \varphi_i(\mathbf{x}_p) \\
        &= -e \psi_p \sum_{i=1}^{N_{CG}} \hat{\phi}_i \nabla \varphi_i(\mathbf{x}_p),
    \end{split}
\end{align}
where $N_{CG}$ is the number of basis functions in the discrete $H^1$ finite element space. Recall that $\mathbf{v}_i \in H(\text{{\normalfont curl}};\Omega)$ are exactly $\nabla \varphi_i$ where $\varphi_i \in H^1(\Omega)$, and recall that in leap-frog stepping, 
\begin{align}
    \begin{split}
        \frac{d\mathbf{x}_p}{dt} & = \frac{\mathbf{p}_p}{m_p}          \\
        \frac{d\mathbf{p}_p}{dt} & = e \psi_p \mathbf{E}(\mathbf{x}_p),
    \end{split}
\end{align}
where $\mathbf{E}$ is given by \eqref{eq:discrete_E_equation},
we see that our implementation is consistent with the canonical dynamics, so the discrete system is indeed symplectic.
