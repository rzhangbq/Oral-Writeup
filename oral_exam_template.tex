...
\end{equation}
\QT{where $f$ is approximated by macro-particle representation as $f \approx \sum_{j=1}^{N_p} \psi_j \delta(\mathbf{x}-\mathbf{X}_j) \delta(\mathbf{p}-\mathbf{P}_j) .$} The electric potential $\phi$ is computed from Poisson's equation:
\begin{equation}
    -\epsilon_0 \Delta\phi(\mathbf{x}) = e \sum_{j=1}^{N_p} \psi_j \delta(\mathbf{x}-\mathbf{X}_j) - e n_0,
\end{equation}
where the vacuum permittivity $\epsilon_0$ is set to 1, $e$ is the particle charge, the particle weight $\psi_j$ is currently set to 1, $\mathbf{X}_j$ is the particle position, and $n_0$ is a constant neutralizing term that enforces global charge neutrality, ensuring that the right-hand side has zero mean and the periodic Poisson problem is well-posed. The electric field is then computed from the potential:
\begin{equation}
    \mathbf{E} = -\nabla \phi.
\end{equation}
...
\begin{equation}
    \epsilon_0 \langle \nabla \varphi, \nabla \phi \rangle = e \sum_{j=1}^{N_p} \psi_j \langle \varphi, \delta(\mathbf{q}-\mathbf{Q}_j) \rangle - e n_0 \langle \varphi, 1 \rangle,
\end{equation}
...
\begin{equation}
    H(\{\mathbf{x}_j\},\{\mathbf{p}_j\})
    = \frac{1}{2}\sum_{j=1}^{N_p}\frac{|\mathbf{p}_j|^2}{m_j}
    + \frac{1}{2\epsilon_0}\hat{\mathbf{B}}(\{\mathbf{x}_j\})^T \mathbf{K}^{-1}\hat{\mathbf{B}}(\{\mathbf{x}_j\}),
\end{equation}
where
\begin{equation}
    \hat{\mathbf{B}}_i(\{\mathbf{x}_j\})
    = e\sum_{j=1}^{N_p}\psi_j\,\varphi_i(\mathbf{x}_j) \;-\; e n_0\int_\Omega \varphi_i.
\end{equation}
